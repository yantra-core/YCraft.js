...
async function uploadContraptions2() {
  try {
    ...
    for (const file of jsFiles) {
      ...
      // inside content, replace the string '../index.js' with `ycraft`
      // this is because the contraption code will be run in the browser, and
      // the browser will not have access to the local folder
      content = content.replace(/'\.\.\/index\.js'/g, "'ycraft'");

      // find any lines that look like: let anyStRinGcase = new YCraft();
